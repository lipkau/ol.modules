...
class ExplorerReplace
{
    ...
    ShowGui()
    {
        ...
        {
            ReplaceDialog := new CReplaceDialog(this)
...
Class CReplaceDialog
{
    __New(Action)
    {
        ...
        Gui, % this.GUINum ":Add",Radio, x160 y10 hwndhFiles gExplorerReplaceDialogFiles, Files
        ...
        Gui, % this.GUINum ":-Resize -MaximizeBox -MinimizeBox +LastFound +LabelExplorerReplaceDialog"
                ...
                if (!this.Stop)
                {
                    ...
                    if (!this.Stop)
                    {
                        Loop % this.SearchResults.MaxIndex()
                        {
                            if (this.Stop)
                                break
                            LV_Add("Check", this.SearchResults[A_Index].Name, StringTrimLeft(this.SearchResults[A_Index].Path,BasePath), this.SearchResults[A_Index].FixedNewFilename ? this.SearchResults[A_Index].FixedNewFilename : this.SearchResults[A_Index].NewFilename)
                        }
                    }
                }
            }
            ...
            if (!this.Stop)
            {
                Loop % this.SearchResults.MaxIndex()
                {
                    ...
                    Loop % this.SearchResults[A_Index].Lines.MaxIndex()
                    {
                        if (this.Stop)
                            break
                        LV_Add("Check", StringTrimLeft(this.SearchResults[index].Path,this.BasePath), this.SearchResults[index].Lines[A_Index].Line, RegexReplace(this.SearchResults[index].Lines[A_Index].Text, "D)(*ANYCRLF)\R$",""), RegexReplace(this.SearchResults[index].Lines[A_Index].NewText, "D)(*ANYCRLF)\R$",""))
                    }
                }
            }
        }
